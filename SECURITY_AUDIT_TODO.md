...
- [x] 🔴 **AUDIT-CRYPTO-001**: ChaCha20 nonce 复用安全性验证
  - **关联代码**: lib.rs:CHACHA_NONCE:99, generate_padding_data:645, generate_filler:662, peel:261, construct_onion_packet:716
  - **审计内容**:
...
- [x] 🔴 **AUDIT-CRYPTO-002**: HMAC 密钥域分离充分性
  - **关联代码**: lib.rs:94-98 (HMAC_KEY_RHO, HMAC_KEY_MU, HMAC_KEY_PAD, HMAC_KEY_UM, HMAC_KEY_AMMAG)
  - **审计内容**:
    - 域分离字符串是否唯一且无前缀冲突
    - 是否与规范一致
  - **现有覆盖**: 测试向量验证
...
- [x] 🔴 **AUDIT-CRYPTO-003**: 恒定时间比较的使用
  - **关联代码**: lib.rs:257 (peel HMAC), lib.rs:367 (parse HMAC)
  - **审计内容**:
    - 所有密码学比较是否使用恒定时间操作
...
- [x] 🟠 **AUDIT-CRYPTO-004**: 椭圆曲线标量有效性验证
  - **关联代码**: lib.rs:608 (derive_next_hop_ephemeral_secret_key), lib.rs:630 (derive_next_hop_ephemeral_public_key)
  - **审计内容**:
...
- [x] 🟠 **AUDIT-CRYPTO-005**: 敏感密钥材料清零
  - **关联代码**: lib.rs 全文（shared_secret, session_key, ephemeral keys）
  - **审计内容**:
    - 共享密钥、派生密钥使用后是否被清零
...
- [x] 🔴 **AUDIT-INPUT-001**: `OnionPacket::from_bytes` 输入验证
  ...
  - **审计内容**:
    ...
    - 公钥有效性验证
    - version 字段验证
  - **现有覆盖**: test_onion_packet_from_bytes
...
- [x] 🔴 **AUDIT-INPUT-003**: `peel()` 中 get_hop_data_len 回调返回值验证
  ...
  - **现有覆盖**: test_peel_returns_error_on_hop_data_len_overflow, test_peel_returns_error_on_hop_data_len_near_packet_data_len
...
- [x] 🟠 **AUDIT-INPUT-004**: `OnionErrorPacket::parse` 输入验证
  ...
  - **审计内容**:
    ...
    - 空 hops_path 处理
    - parse_payload 回调安全性
  - **现有覆盖**: test_parse_onion_error_packet
...
- [x] 🔴 **AUDIT-MEMORY-001**: 整数溢出/下溢保护
  - **关联代码**: lib.rs:268-273, 668-672, 709-711, 722
  - **审计内容**:
    - 所有算术运算的溢出保护
    - 切片索引的边界检查
  - **现有覆盖**: test_peel_returns_error_on_hop_data_len_overflow
...
- [x] 🟠 **AUDIT-MEMORY-004**: 资源耗尽风险
  - **关联代码**: lib.rs:158-186 (create), lib.rs:575-580 (forward_stream_cipher)
  - **审计内容**:
...
- [x] 🔴 **AUDIT-LOGIC-001**: 洋葱包构造正确性
  - **关联代码**: lib.rs:697-736 (construct_onion_packet)
  - **审计内容**:
    - 反向迭代逻辑正确性
...
- [x] 🟠 **AUDIT-SERDE-001**: OnionPacket 序列化/反序列化 roundtrip
  - **关联代码**: lib.rs:189-216 (into_bytes, from_bytes)
  - **审计内容**:
    - roundtrip 一致性
    - 畸形/截断数据处理
  - **现有覆盖**: test_onion_packet_from_bytes
...
- [x] 🟠 **AUDIT-SERDE-002**: OnionErrorPacket split/concat roundtrip
  - **关联代码**: lib.rs:310-398 (concat, split, into_bytes, from_bytes)
  - **审计内容**:
    - roundtrip 一致性
    - 短包（< 32 字节）处理
...
- [x] 🟠 **AUDIT-ERRINFO-001**: 错误类型信息泄露
  - **关联代码**: lib.rs:400-422 (SphinxError)
  - **审计内容**:
    - 不同错误类型是否可被攻击者利用
    - 是否存在 oracle 风险
  - **现有覆盖**: 无专门测试
...
- [x] 🟢 **AUDIT-DEPS-001**: 依赖 CVE 检查
  - **关联代码**: Cargo.toml
  - **审计内容**:
    - secp256k1 0.30.0, sha2 0.10.8, hmac 0.12.1, chacha20 0.9.1, thiserror 1.0, subtle 2.6
    - 是否存在已知漏洞
  - **现有覆盖**: N/A
