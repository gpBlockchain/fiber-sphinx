...
impl OnionPacket {
    ...
    pub fn create<C: Signing>(
        ...
        let pad_key = Zeroizing::new(derive_key(HMAC_KEY_PAD, &session_key.secret_bytes()));
        ...
        F: FnOnce(&[u8]) -> Option<usize>,
    {
        ...
        let shared_secret = Zeroizing::new(self.shared_secret(secret_key));
        let rho = Zeroizing::new(derive_key(HMAC_KEY_RHO, shared_secret.as_ref()));
        let mu = Zeroizing::new(derive_key(HMAC_KEY_MU, shared_secret.as_ref()));
...
impl OnionErrorPacket {
    ...
    pub fn create(shared_secret: &[u8; 32], payload: Vec<u8>) -> Self {
        let keys = ReturnKeys::new(shared_secret);
        let hmac = compute_hmac(&keys.um, &payload, None);
        Self::concat(hmac, payload).xor_cipher_stream_with_ammag(keys.ammag)
    }
    ...
    pub fn xor_cipher_stream(self, shared_secret: &[u8; 32]) -> Self {
        let ammag = Zeroizing::new(derive_ammag_key(shared_secret));
        ...
        F: Fn(&[u8]) -> Option<T>,
    {
        ...
        {
            let keys = ReturnKeys::new(&shared_secret);
            packet = packet.xor_cipher_stream_with_ammag(keys.ammag);
            // Verify HMAC before parsing payload (authenticate then parse)
            let hmac = compute_hmac(&keys.um, &packet.packet_data[32..], None);
